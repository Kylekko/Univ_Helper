...
public class Cafe extends AppCompatActivity {
    ...
    private ArrayList<Cafe.ItemObject> list = new ArrayList();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        appFragment.getMapAsync(new OnMapReadyCallback() {
            ...
            public void onMapReady(@NonNull GoogleMap googleMap) {
                Cafe.this.googleMap=googleMap;
                LatLng latLng = new LatLng(36.84082025252936,127.18046587264169);
                googleMap.moveCamera(CameraUpdateFactory.newLatLng(latLng));
                googleMap.moveCamera(CameraUpdateFactory.zoomTo(12));


                myItemClusterManager = new ClusterManager<MyItem>(Cafe.this,googleMap);
                ...
                myItemClusterManager.setOnClusterItemClickListener(new ClusterManager.OnClusterItemClickListener<MyItem>() {
                    ...
                    public boolean onClusterItemClick(MyItem item) {
                        switch(item.getTitle()){
                            case "육회한날":
                                Toast.makeText(getApplicationContext(),"육회한날",Toast.LENGTH_SHORT).show();
                                return true;
                            case "한라맥주":
                                Toast.makeText(getApplicationContext(),"한라맥주",Toast.LENGTH_SHORT).show();
                                return true;
                            case "장미회관":
                                Toast.makeText(getApplicationContext(),"장미회관",Toast.LENGTH_SHORT).show();
                                return true;
                            case "딥인싸이드":
                                Toast.makeText(getApplicationContext(),"딥인싸이드",Toast.LENGTH_SHORT).show();
                                return true;
    ...
    private class Description extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPreExecute() {
            ...
            progressDialog = new ProgressDialog(Cafe.this);
        ...
        protected Void doInBackground(Void... params) {
            ...
            try {
                ...
                for(Element elem : mElementDataSize) {
                    ...
                    list.add(new Cafe.ItemObject(my_title, my_reward, my_place, my_view));
                }
        ...
        protected void onPostExecute(Void result) {
            CrawlingAdapter2 myAdapter = new CrawlingAdapter2(list);
    ...
    private void addItems(){
        double lat[] = new double[]{36.84081057700755,36.84043271052631,36.842429438117236,36.841135229524866};
        double lng[] = new double[]{127.18090308426909   ,127.18050980638733  ,127.18294736740182   ,127.18073568117393   };
        String name[] = new String[]{"공차(카페)","이디야커피(카페)","피카플레이스(카페)","블랙컨테이너(카페)"};
        for(int i=0 ; i<4; i++) {
